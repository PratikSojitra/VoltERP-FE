...
export default function CreateInvoicePage() {
    ...
    const { useGetNextInvoiceNumber, useCreateInvoice } = useInvoices();
    const { data: nextNumberData } = useGetNextInvoiceNumber(user?.id);
    ...
    const onSubmit = (data: InvoiceFormValues) => {
        ...
        const itemsWithTotals = data.items.map(item => {
            ...
            return {
                ...
                inventoryIDU: undefined, // Strip temp fields
        ...
        <div className="max-w-7xl mx-auto space-y-6 pb-20">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight text-foreground">Create Sales Invoice</h2>
                        <p className="text-muted-foreground text-sm">Draft a new sales invoice to bill your customer.</p>
                    </div>
                </div>
                                                                        ...
                                                                        <FormMultiCombobox
                                                                            ...
                                                                            options={inventories
                                                                                .filter((inv: any) => {
                                                                                    ...
                                                                                    return prodId === watchItems[index].product && inv.status === 'IN_STOCK' && (uType.includes('Indoor') || uType.includes('IDU'));
                                                                                })
                                                                                .map((inv: any) => ({
                                                                        ...
                                                                        <FormMultiCombobox
                                                                            ...
                                                                            options={inventories
                                                                                .filter((inv: any) => {
                                                                                    const prodId = typeof inv.product === 'object' ? inv.product?._id : inv.product;
                                                                                    const uType = inv.unitType || "";
                                                                                    return prodId === watchItems[index].product && inv.status === 'IN_STOCK' && (uType.includes('Outdoor') || uType.includes('ODU'));
                                                                                })
                                                                                .map((inv: any) => ({
                                                                                    label: `SN: ${inv.serialNumber}`,
                                                                                    value: inv._id
