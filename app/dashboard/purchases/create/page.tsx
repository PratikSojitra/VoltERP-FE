...
export default function CreatePurchasePage() {
    ...
    const { useCreatePurchase, useGetNextPurchaseNumber } = usePurchases();

    const { data: vendorsData } = useGetVendors(1, 100);
    const { data: productsData } = useGetProducts(1, 100);
    const createMutation = useCreatePurchase();
    const { data: nextNumberData, isLoading: isLoadingNextNumber } = useGetNextPurchaseNumber();
    ...
    // Pre-fill purchase number once fetched
    useEffect(() => {
        if (nextNumberData?.invoiceNumber) {
            setValue("invoiceNumber", nextNumberData.invoiceNumber);
        }
        ...
        <div className="max-w-7xl mx-auto space-y-6 pb-20">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold tracking-tight text-foreground">Create Purchase Invoice</h2>
                        <p className="text-muted-foreground text-sm">Add a new purchase to record supplier bills, taxes, and stock.</p>
                    </div>
                </div>
                <div className="flex gap-3">
                    <Button variant="outline" onClick={() => router.back()}>Cancel</Button>
                    <Button onClick={handleSubmit(onSubmit as any)} disabled={createMutation.isPending} className="gap-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1.5">
                            <Label className="flex items-center gap-2">
                                Purchase Number <span className="text-destructive">*</span>
                                {isLoadingNextNumber && <Loader2 className="w-3 h-3 animate-spin text-muted-foreground" />}
                            </Label>
                            <Input
                                {...register("invoiceNumber")}
                                className={`font-mono ${errors.invoiceNumber ? 'border-destructive' : ''}`}
                                placeholder={isLoadingNextNumber ? "Generating..." : ""}
                            />
                            {errors.invoiceNumber && <p className="text-xs text-destructive">{errors.invoiceNumber.message}</p>}
                            <p className="text-[11px] text-muted-foreground">Auto-generated suggestion — you can override this if needed.</p>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-5 relative">
                    <div className="sticky top-6 rounded-xl border border-border bg-[var(--sidebar-background)] p-6 shadow-sm flex flex-col gap-4">
                        ...
                        <Button size="lg" className="w-full mt-6 gap-2 text-md" onClick={handleSubmit(onSubmit as any)} disabled={createMutation.isPending}>
